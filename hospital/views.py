...
def patient_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and patientForm.is_valid():
            ...
            patient.status=False  # Approval required
            patient.save()
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and patientForm.is_valid():
            try:
                ...
                doctor = patientForm.cleaned_data.get('assignedDoctorId')
                if doctor:
                    patient.assignedDoctorId = doctor
...
def doctor_dashboard_view(request):
    ...
    #for recent patients table in doctor dashboard
    patients=models.Patient.objects.all().filter(status=True,assignedDoctorId=request.user.id).order_by('-id')[:10]
    mydict={
    'patientcount':patientcount,
    'appointmentcount':appointmentcount,
...
def patient_dashboard_view(request):
    ...
    mydict = {
        ...
        'symptoms': patient.symptoms,
        'admitDate': patient.admitDate,
        'patientStatus': 'Active' if patient.status else 'Pending Approval',
    }
